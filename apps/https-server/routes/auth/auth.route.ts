...
router.post("/signin",genricRateLimiter(15, 300), signIn);
router.post("/forgot",genricRateLimiter(600, 30), forgotPassword);
router.post("/reset",genricRateLimiter(600, 30), resetPassword);
router.post("/logout",genricRateLimiter(60, 3000), logout);
router.post("/refresh",genricRateLimiter(15, 300), refresh);
router.get("/session",genricRateLimiter(15, 600),authenticate,getSession)
router.get("/csurf",genricRateLimiter(15, 600), csurf);
router.get("/google",genricRateLimiter(15, 600), startGoogleAuth());
router.get("/github",genricRateLimiter(15, 600), startGithubAuth());

router.get("/google/callback",genricRateLimiter(15, 600), googleCallbackMiddleware, handleGoogleCallback);
