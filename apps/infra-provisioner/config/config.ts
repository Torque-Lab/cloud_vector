...
const OPS_VM_PAT=process.env.OPS_VM_PAT;
...
export const repoUrlVmWithPAT = `https://${OPS_VM_PAT}@github.com/Torque-Lab/vm-tf.git`; 
export const repoUrlVmWithOutPAT="https://github.com/Torque-Lab/vm-tf.git"

export function vmRepoPath(){
    if(!fs.existsSync(`${process.cwd()}/vm_temp_dir`)){
        fs.mkdirSync(`${process.cwd()}/vm_temp_dir`);
    }   
    return `${process.cwd()}/vm_temp_dir`;
}